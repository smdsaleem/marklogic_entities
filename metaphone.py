...
def get_company_dm(name, return_all=False):
    import itertools
    """dm(string) -> [(string, string or None), (string, string or None), ...]
        returns the double metaphone codes for given string - always a tuple list
        input string should be whitespace separated"""
    namelist = name.split()
    rstlist = []
    for item in namelist:
        if contains_digits(item):
            newlist = ["".join(x) for _, x in itertools.groupby(item, key=str.isdigit)]
            for newitem in newlist:
                if newitem.isdigit():
                    rstlist.append(newitem)
                else:
                    rstlist.append(dm(newitem)[0])
        #proc_digit
        else:
            rstlist.append(dm(item)[0])
    if return_all:
        return rstlist
    else:
        return " ".join([item for item in rstlist if item!=""]).strip()
...
if __name__ == '__main__' :
    ...
    pairs = [("Romolus", "Romulus"), ("Amcore","Amcor"), ("astrazeneka", "AstraZeneca"), ("123 Company", "2 Company"), ("Amkor limited", "Amcor limited"), ("4Digital","Digital1"), ("Di@g@ital","!Digital"),("Cs2 Advertising", "Cs 2 Advertising")]
    for pair in pairs:
        print '"%s" double metaphone code is "%s"' %(pair[0], get_company_dm(pair[0]))
        print '"%s" double metaphone code is "%s"' %(pair[1], get_company_dm(pair[1]))
    '''
    #f = open("/KPMG0005-1/linzi.zhang/data/digitname_test.txt")
    #namelist = []
    #for line in f.readlines():
    #    namelist.append(line.strip())
    #namelist = ["maikurosohuto","HH","HWI","WW","HWA"]
    #namelist = ["premier league","premere legue","premeer leeg","WW","HWA"]
    namelist = ["chevron","stanley","premeer leeg","WW","HWA"]
    for name in namelist[:2]:
        print '"%s" double metaphone code is "%s"' %(name, get_company_dm(name))
